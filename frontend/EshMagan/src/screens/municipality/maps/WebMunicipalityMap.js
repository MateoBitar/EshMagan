...
export default function WebMunicipalityMap({
  ...
  const handleRecenter = () => {
    ...
    if (municipalityCoords && isValidCoordPair(municipalityCoords.lat, municipalityCoords.lng)) {
      const ll = L.latLng(municipalityCoords.lat, municipalityCoords.lng);
      combinedBounds = combinedBounds ? combinedBounds.extend(ll) : L.latLngBounds([ll, ll]);
    }
    ...
    responders.forEach(r => {
      if (!r.unitCoords || !isValidCoordPair(r.unitCoords.lat, r.unitCoords.lng)) return;
      const ll = L.latLng(r.unitCoords.lat, r.unitCoords.lng);
      combinedBounds = combinedBounds ? combinedBounds.extend(ll) : L.latLngBounds([ll, ll]);
    });

    validFires.forEach(f => {
      const severityLevel = Number(f.fire_severitylevel ?? f.severity ?? 0);
      const radius = getFireZoneRadiusMeters(severityLevel);
